...
def extract_hog(samples, logger):
    ...
    train = np.resize(train, (total, 3780))#给定一个数组，和特定维度，将会返回一个给定维度形式的新数组
...
def test_hog_detect(test, svm_detector, logger):
    ...
    for f in test:
        ...
        rects, _ = hog.detectMultiScale(img, winStride=(4,4), padding=(8,8), scale=1.05)#图像进行多尺度检测
        for (x,y,w,h) in rects:
            cv2.rectangle(img, (x,y), (x+w,y+h), (0,0,255), 2)#（x，y）是矩阵的左上点坐标 x+w，y+h ）是矩阵的右下点坐标 0,255,0）是画线对应的rgb颜色2是所画的线的宽度
        cv2.imshow('Detect', img)
        c = cv2.waitKey(0) & 0xff
        if c == 27:
            break
    cv2.destroyAllWindows()
'''def test_ag(test,svm_detector):
    hog = cv2.HOGDescriptor()
    hog.setSVMDetector(cv2.HOGDescriptor_getDefaultPeopleDetector())
    pwd = os.getcwd()
    test_dir = os.path.join(pwd, 'Negative')
    print("test_dir")
    i = 0
    j = 0
    m = 0
