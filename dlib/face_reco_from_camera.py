...
import age.birth_predict as ab
import video_emotion_color_demo as vecd
import time
...
while cap.isOpened():
    ...
    kk = cv2.waitKey(1)
    age=ab.predict_birth(img_rd)
    age=str(age)
    emotion=vecd.emotion(img_rd)
    # 取灰度
            ...
            for k in range(len(faces)):
                ...
                if min(e_distance_list) < 0.4:
                    ...
                    name_namelist[k] = str("Person "+str(int(similar_person_num)+1))\
                        .replace("Person 1", "pangxiaowen")\
                        .replace("Person 2", "guoqiyang")\
                        .replace("Person 3", "hanjun")\
    ...
    fps = 1 / (sec)
    strs = 'FPS: %2.3f' % fps
    text_fps_x = len(img_rd[0]) - 150
    text_fps_y = 20
    cv2.putText(img_rd, strs, (text_fps_x, text_fps_y),
    cv2.FONT_HERSHEY_COMPLEX_SMALL, 1, (0, 0, 255), thickness=1, lineType=2)
    cv2.putText(img_rd, "Press 'q': Quit", (20, 450), font, 0.8, (84, 255, 159), 1, cv2.LINE_AA)
    cv2.putText(img_rd, "Face Recognition", (20, 40), font, 1, (0, 0, 0), 1, cv2.LINE_AA)
    cv2.putText(img_rd, "Faces: " + str(len(faces)), (20, 100), font, 1, (0, 0, 255), 1, cv2.LINE_AA)
    cv2.putText(img_rd, age, (20, 350), font, 0.8, (0, 0, 255), 1, cv2.LINE_AA)
    cv2.putText(img_rd, emotion,(20, 400), font, 0.8, (0, 0, 255), 1, cv2.LINE_AA)
